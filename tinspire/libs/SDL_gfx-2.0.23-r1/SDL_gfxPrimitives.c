...
int aaellipseColor(SDL_Surface * dst, Sint16 x, Sint16 y, Sint16 rx, Sint16 ry, Uint32 color)
{
	...
	*/
	...
	od = (Sint16)rint(sab*0.01) + 1; /* introduce some overdraw */
	dxt = (Sint16)rint((double)a2 / sab) + od;
	...
	dyt = (Sint16)rint((double)b2 / sab ) + od;    
...
int bezierColor(SDL_Surface * dst, const Sint16 * vx, const Sint16 * vy, int n, int s, Uint32 color)
{
	...
	*/
	result = 0;
	t=0.0;
	x1=(Sint16)rint(_evaluateBezier(x,n+1,t));
	y1=(Sint16)rint(_evaluateBezier(y,n+1,t));
	for (i = 0; i <= (n*s); i++) {
		t += stepsize;
...
{	
	...
	if (m->oct2 == 0) {
		ptx = x1 + (Sint16)rint(offset * sang);
		if (m->quad4 == 0) {
			pty = y1 - (Sint16)rint(offset * cang);
		} else {
			pty = y1 + (Sint16)rint(offset * cang);
		}
	} else {
		ptx = x1 - (Sint16)rint(offset * cang);
		if (m->quad4 == 0) {
			pty = y1 + (Sint16)rint(offset * sang);
		} else {
			pty = y1 - (Sint16)rint(offset * sang);
		}
	}
